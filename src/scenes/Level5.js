...
export class Level5 extends Phaser.Scene {
	...
	addPlarforms() {
		...
		this.platforms
			...
			.setTint(0x21842f)
			.refreshBody();
		this.platforms.create(410, 390, "ground2").setTint(0x21842f).setScale(0.6).refreshBody();
		this.horizontalPlatform = this.platforms.create(250, 330, "ground3").setTint(0x21842f).setScale(0.6).refreshBody();
		this.platforms.create(440, 260, "ground2").setTint(0x21842f).setScale(0.6).refreshBody();
		this.platforms.create(-10, 260, "ground2").setTint(0x21842f).setScale(0.6).refreshBody();
		...
		this.horizontalaereo = this.platforms.create(410, 130, "aereo").setTint(0x21842f).setScale(0.5).refreshBody() ;
		this.platforms.create(120, 65, "ground3").setTint(0x21842f).setScale(0.6).refreshBody() ;
	}

	addEnigmas() {
		this.enigmas = this.physics.add.group();

		this.enigmas.create(400, 345, "enigma").postFX.addGlow(0xFF0000, 0.5, 0.7); // 3
		this.enigmas.create(30, 190, "enigma").postFX.addGlow(0x00FF00, 0.5, 0.7); // 1
		this.enigmas.create(120, 20, "enigma").postFX.addGlow(0x0000FF, 0.5, 0.7); // 2 
	...
	update() {
		...
		if (this.horizontalaereo.x >= maxX) {
			this.horizontalaereo.flipX = true;
		} else if (this.horizontalaereo.x <= minX) {
			this.horizontalaereo.flipX = false;
		}
	
		if (this.horizontalaereo.flipX) {
			this.horizontalaereo.x -= speed;
		} else {
			this.horizontalaereo.x += speed;
		}

		this.horizontalaereo.body.position.x = this.horizontalaereo.x-60;
